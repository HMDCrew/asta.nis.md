	...
	class ASTA_THEME_GET_AUCTION {
		...
		private function adition_card_requirements( array $posts, int $curent_user_id ) {
			...
			foreach ( $posts as $key => $post ) {
				...
				$posts[ $key ]['auction_date']    = ASTA_AUCTION::get_auction_date( $post['ID'] );
				$posts[ $key ]['price']           = ASTA_AUCTION::get_auction_last_price( $post['ID'] );
				$posts[ $key ]['price_increment'] = asta_esc_meta( $post['ID'], 'price_increment' );
